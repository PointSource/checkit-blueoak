...
# Injects the Google client ID into the client/src/app.config.json file
sed -i "s|SET_GOOGLE_CLIENT_ID_HERE|$GOOGLE_CLIENT_ID|g" $WORKSPACE/server/config/default.json
...
sed -i "s|SET_DECRYPTION_PASSWORD_HERE|$GOOGLE_DOMAIN|g" $WORKSPACE/server/config/default.json
# Injects the service account email into the client/src/app.config.json file
sed -i "s|SET_DECRYPTION_PASSWORD_HERE|$SERVICE_ACCOUNT_EMAIL|g" $WORKSPACE/server/config/default.json
# Injects the service account key file path into the client/src/app.config.json file
sed -i "s|SET_DECRYPTION_PASSWORD_HERE|$SERVICE_ACCOUNT_KEY_FILE_PATH|g" $WORKSPACE/server/config/default.json
# Injects the app account email into the client/src/app.config.json file
sed -i "s|SET_DECRYPTION_PASSWORD_HERE|$APP_ACCOUNT_EMAIL|g" $WORKSPACE/server/config/default.json
